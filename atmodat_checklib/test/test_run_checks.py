...
def create_output_summary(opath):
    ...
    json_summary_out, cf_errors_out = {}, {}
    files = output_directory.return_files_in_directory_tree(opath)
    for file in files:
        if file.endswith("_result.json"):
            json_summary_out[file] = pd.DataFrame()
            json_summary_out[file] = json_summary_out[file].append(extract_overview_output_json(file), ignore_index=True)
        elif file.endswith("_CF_result.txt"):
            cf_errors_out[file] = 0
            cf_errors_out[file], std_name_table = extracts_error_summary_cf_check(file, cf_errors_out[file])
    return json_summary_out, cf_errors_out


def run_checks_on_files(ifiles):
    for check in ['atmodat', 'CF']:
        for f in os.listdir('tmp/' + check):
            os.remove('tmp/' + check+'/' + f)
    run_checks(ifiles, False, ['atmodat', 'CF'], 'auto', 'tmp/',
               '/home/jkretzs/work/scripts/python/atmodat/atmodat_standard_checker/')
    json_summary, cf_errors = create_output_summary('tmp/')
...
def test_expected_attributes_present():
    file_list = glob.glob('demo_data/*.nc')
    passed_json_checks = run_checks_on_files(file_list)
